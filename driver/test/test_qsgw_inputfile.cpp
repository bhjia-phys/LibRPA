...
void test_hartree_modes_are_qsgw_only_and_explicit()
{
    parse(valid_qsgw_prefix() +
          "qsgw_update_hartree = true\n"
          "qsgw_hartree_coulomb = TRUNCATED\n"
          "qsgw_hartree_normalization = LEGACY_EXTRA_INVERSE_NK\n");
    assert(driver::driver_params.qsgw_update_hartree);
    assert(driver::driver_params.qsgw_hartree_coulomb == "truncated");
    assert(driver::driver_params.qsgw_hartree_normalization ==
           "legacy_extra_inverse_nk");
    const std::string formatted = driver::driver_params.format();
    assert(formatted.find("qsgw_update_hartree = true") !=
           std::string::npos);
    assert(formatted.find("qsgw_hartree_coulomb = truncated") !=
           std::string::npos);
    ...
    assert_throws([&] {
        parse(valid_qsgw_prefix() +
              "qsgw_hartree_coulomb = screened\n");
    });
    assert_throws([&] {
        parse(valid_qsgw_prefix() +
              "qsgw_hartree_normalization = double_counted\n");
    });
}
