...
source_repo_url=https://github.com/AroundPeking/abacus-develop.git
source_branch=master_ghj
pinned_commit=dd4216653386d32f79e3219f3ea5dd2d229c1c5a
run_root=/home/bhj/ai-runs/abacus-pinned-dd421665-source-20260720-v1
source_root=$run_root/source

test ! -e "$run_root"
mkdir -p "$run_root"
cp "$0" "$run_root/freeze_abacus_source_on_fish_v1.sh"

git ls-remote "$source_repo_url" "refs/heads/$source_branch" \
  >"$run_root/ls-remote.txt"
test "$(awk '{print $1}' "$run_root/ls-remote.txt")" = "$pinned_commit"

git clone --branch "$source_branch" --single-branch \
  "$source_repo_url" "$source_root" \
...
test "$(git -C "$source_root" remote get-url origin)" = "$source_repo_url"
