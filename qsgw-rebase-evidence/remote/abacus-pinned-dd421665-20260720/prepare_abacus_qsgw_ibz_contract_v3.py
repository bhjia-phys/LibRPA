#!/usr/bin/env python3
"""Generate a strict QSGW contract for state-basis ABACUS out_mat_xc files."""
...
def read_abacus_native_vxc_dimension(path: Path) -> int:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if match:
            ...
            if key in dimensions:
                raise ValueError(f"duplicate native ABACUS Vxc {key} header: {path}")
            dimensions[key] = int(match.group(2))
    if set(dimensions) != {"rows", "columns"}:
        raise ValueError(f"incomplete native ABACUS Vxc dimensions: {path}")
    if dimensions["rows"] <= 0 or dimensions["rows"] != dimensions["columns"]:
        raise ValueError(f"invalid native ABACUS Vxc dimensions: {path}")
...
def write_vxc_manifest(
    ...
    legacy_aliases = sorted(dataset.glob("vxcs1k*_nao.txt"))
    if legacy_aliases:
        raise ValueError(
            "producer-native QSGW contract refuses derived vxcs1k aliases: "
            + ", ".join(path.name for path in legacy_aliases)
        )
    matrices = indexed_files(
        dataset, r"vxck(\d+)_nao\.txt", list(range(1, len(kpoints) + 1))
    )
    lines = [
        VXC_MAGIC,
        "kind scf",
    ...
    for k_index, (kpoint, matrix) in enumerate(zip(kpoints, matrices), start=1):
        if read_abacus_native_vxc_dimension(matrix) != n_bands:
            raise ValueError(
                f"Vxc state dimension does not match band_out n_bands: {matrix}"
            )
        lines.append(
