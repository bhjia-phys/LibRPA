...
grep -v -E '  (CITATIONS\.md|tools/generate_orbital\.sh)$' \
    "$artifacts/TRACKED_SOURCE_SHA256SUMS.txt" \
    > "$root/TRACKED_REGULAR_SOURCE_SHA256SUMS.txt"
(
    cd "$source_dir"
    sha256sum -c "$root/TRACKED_REGULAR_SOURCE_SHA256SUMS.txt"
) > "$root/main-manifest-check.stdout" 2> "$root/main-manifest-check.stderr"

(
    cd "$source_dir"
    test -L CITATIONS.md
    test "$(readlink CITATIONS.md)" = 'docs/CITATIONS.md'
    test "$(printf '%s' "$(readlink CITATIONS.md)" | sha256sum | awk '{print $1}')" \
        = '252b3c02838fd55073734b661ee49316fab3def2a6764bbc9d997bddf2950a81'
    test -L tools/generate_orbital.sh
    test "$(readlink tools/generate_orbital.sh)" = 'SIAB/Generate_Orbital_AllInOne.sh'
    test "$(printf '%s' "$(readlink tools/generate_orbital.sh)" | sha256sum | awk '{print $1}')" \
        = 'c78d9c6a432f2e95e6d26750b01045b87f12c699ce0d108fb67b5123261f4042'
    printf '%s\n' \
        'CITATIONS.md -> docs/CITATIONS.md: OK' \
        'tools/generate_orbital.sh -> SIAB/Generate_Orbital_AllInOne.sh: OK'
) > "$root/main-symlink-check.stdout" 2> "$root/main-symlink-check.stderr"
...
find "$source_dir" -type l -printf '%P -> %l\n' | LC_ALL=C sort > "$root/PREPARED_SYMLINK_INVENTORY.txt"
sha256sum "$root/PREPARED_FILE_INVENTORY.txt" "$root/PREPARED_SYMLINK_INVENTORY.txt" \
    > "$root/PREPARED_INVENTORIES.sha256"
touch "$root/SOURCE_PREPARED"

sha256sum \
    "$root/SOURCE_PROVENANCE.txt" \
    "$root/PREPARED_FILE_INVENTORY.txt" \
    "$root/PREPARED_SYMLINK_INVENTORY.txt" \
    "$root/main-manifest-check.stdout" \
    "$root/main-manifest-check.stderr" \
    "$root/main-symlink-check.stdout" \
    "$root/main-symlink-check.stderr" \
...
cat "$root/main-symlink-check.stdout"
cat "$root/PREPARED_INVENTORIES.sha256"
