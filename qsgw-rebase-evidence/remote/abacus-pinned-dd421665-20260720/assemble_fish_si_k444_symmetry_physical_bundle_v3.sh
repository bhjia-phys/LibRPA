...
root=$base/abacus-pinned-dd421665-si-k444-symmetry-physical-bundle-20260723-v3
...
runner_relative=qsgw-rebase-evidence/remote/abacus-pinned-dd421665-20260720/assemble_fish_si_k444_symmetry_physical_bundle_v3.sh
builder_relative=qsgw-rebase-evidence/remote/abacus-pinned-dd421665-20260720/build_abacus_physical_stru_overlay_v1.py
bz_builder_relative=qsgw-rebase-evidence/remote/abacus-pinned-dd421665-20260720/build_abacus_physical_bz_sampling_overlay_v1.py
...
for relative in "$builder_relative" "$bz_builder_relative" "$basis_relative" "$contract_relative"; do
...
while IFS= read -r -d '' path; do
  name=$(basename "$path")
  case "$name" in
    stru_out|bz_sampling_out|basis_wfc_out|basis_aux_out|basis_out|basis_metadata_summary.json|\
...
python3 -B "$tool_dir/build_abacus_physical_bz_sampling_overlay_v1.py" \
  "$dataset/stru_out" "$source_dataset/bz_sampling_out" \
  "$dataset/bz_sampling_out" "$provenance/PHYSICAL_BZ_SAMPLING_OVERLAY.json" \
  >"$provenance/physical-bz-sampling-overlay.stdout"
python3 -B - "$provenance/PHYSICAL_BZ_SAMPLING_OVERLAY.json" <<'PY'
import json
import sys
...
assert report["scf_kpoint_count"] == 8
assert report["ibz_kpoint_count"] == 8
assert report["non_cartesian_tokens_unchanged"] is True
assert report["fractional_recovery_max_abs"] <= 1.0e-12
PY
...
gate=fish_pinned_abacus_si_k444_symmetry_physical_bundle_v3
...
source_bz_sampling_out_sha256=$(sha256sum "$source_dataset/bz_sampling_out" | awk '{print $1}')
physical_bz_sampling_out_sha256=$(sha256sum "$dataset/bz_sampling_out" | awk '{print $1}')
...
echo FISH_SI_K444_SYMMETRY_PHYSICAL_BUNDLE_V3=PASS
