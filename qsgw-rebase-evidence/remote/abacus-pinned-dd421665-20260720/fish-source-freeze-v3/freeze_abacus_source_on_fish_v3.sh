...
failed_v2=/home/bhj/ai-runs/abacus-pinned-dd421665-source-20260720-v2
run_root=/home/bhj/ai-runs/abacus-pinned-dd421665-source-20260720-v3
submodule_path=deps/libpaw_interface
submodule_commit=c211c0ab330adf3cc374f50ab3edee46b174e64c
submodule_root=$source_root/$submodule_path

test ! -e "$run_root"
test -d "$failed_v2"
test ! -e "$failed_v2/SOURCE_FROZEN"
test -d "$source_root/.git"
mkdir -p "$run_root"
cp "$0" "$run_root/freeze_abacus_source_on_fish_v3.sh"
...
git -C "$source_root" submodule update --init --recursive \
  >"$run_root/submodule-update.stdout" \
  2>"$run_root/submodule-update.stderr"
test -d "$submodule_root/.git" || test -f "$submodule_root/.git"
test "$(git -C "$submodule_root" rev-parse HEAD)" = "$submodule_commit"
test -z "$(git -C "$submodule_root" status --porcelain --untracked-files=all)"
...
git -C "$source_root" submodule status --recursive \
  >"$run_root/submodule-status.txt"
git -C "$source_root" ls-files -s >"$run_root/GIT_INDEX.txt"
git -C "$source_root" ls-tree -r --full-tree HEAD \
  >"$run_root/GIT_TREE.txt"
grep '^160000 ' "$run_root/GIT_TREE.txt" >"$run_root/GITLINKS.txt"
grep -Fq "$submodule_commit" "$run_root/GITLINKS.txt"
...
) >"$run_root/LIBPAW_TRACKED_SOURCE_SHA256SUMS.txt"
git -C "$submodule_root" log -1 --format=fuller \
  >"$run_root/libpaw-commit.txt"
git -C "$submodule_root" remote -v >"$run_root/libpaw-remotes.txt"
...
git -C "$source_root" archive --format=tar.gz \
  ...
  "$pinned_commit"
git -C "$submodule_root" archive --format=tar.gz \
  --prefix=libpaw_interface-c211c0ab/ \
  --output="$run_root/libpaw_interface-c211c0ab-source.tar.gz" \
  "$submodule_commit"
...
du -h "$run_root/abacus-master_ghj-dd421665.bundle" \
  ...
  "$run_root/libpaw_interface-c211c0ab-source.tar.gz" \
...
submodule_archive_sha=$(awk 'NR == 3 {print $1}' "$run_root/FROZEN_SOURCE_SHA256SUMS.txt")
manifest_sha=$(awk 'NR == 4 {print $1}' "$run_root/FROZEN_SOURCE_SHA256SUMS.txt")
submodule_manifest_sha=$(awk 'NR == 5 {print $1}' "$run_root/FROZEN_SOURCE_SHA256SUMS.txt")
printf '%s\n' \
  ...
  "submodule_archive_sha256=$submodule_archive_sha" \
  "tracked_manifest_sha256=$manifest_sha" \
  "submodule_manifest_sha256=$submodule_manifest_sha" \
