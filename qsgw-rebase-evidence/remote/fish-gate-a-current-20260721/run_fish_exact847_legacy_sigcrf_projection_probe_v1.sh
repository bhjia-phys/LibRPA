...
for tool in \
  ...
  classify_exact847_sigma_projection_probe_v1.py; do
...
gate=gate_a1_exact847_legacy_sigcrf_projection_probe_v1
...
PYTHONPATH="$tools_dir" "$python" -B \
  ...
  "$run_root/legacy-sigcrf-candidate-component-comparison.json" \
...
"$python" -B "$tools_dir/classify_exact847_sigma_projection_probe_v1.py" \
  "$run_root/legacy-sigcrf-candidate-component-comparison.json" \
  "$run_root/sigma-projection-classification.json" \
...
projection_classification_sha256=$(sha256sum "$run_root/sigma-projection-classification.json" | awk '{print $1}')
...
cat "$run_root/sigma-projection-classification.json"
run_succeeded=1
trap - EXIT
