...
runner_relative=qsgw-rebase-evidence/remote/fish-formal-gate-a-20260723/gate-a23-current-symmetry-side-v4.sh
...
test -x "$python"
mkdir -p "$tool_dir"
cp "$0" "$run_root/gate-a23-current-symmetry-side-v4.sh"
test "$(sha256sum "$run_root/gate-a23-current-symmetry-side-v4.sh" | awk '{print $1}')" = \
  "$RUNNER_SHA256"
...
echo CURRENT_QSGW_SYMMETRY_SIDE_V4=PASS_PENDING_FULL_BZ_CONTROL
