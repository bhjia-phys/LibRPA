#!/usr/bin/env python3
"""Compare corrected legacy-v4 and current-v6 QSGW Hartree traces.

The numerical aligner understands legacy v4 versus current v5. This adapter
first validates the actual Hartree-on v4/v6 contracts, then rewrites only the
v6 version and full-BZ symmetry labels so the aligner can compare unchanged
numeric rows. Both implementations must use the legacy extra inverse-Nk
normalization and the same linear mixing beta.
"""
...
def compare(args: argparse.Namespace) -> dict[str, object]:
    ...
    numeric = base_module.compare_legacy_v4_current_v5(
        ...
        expected_mixing_beta=args.expected_legacy_beta,
        expected_current_mixer=args.expected_mode,
        expected_hartree=True,
        expected_use_shrink_abfs=False,
    )
    ...
    return {
        ...
        "contract_mode": "legacy_hartree_v4_to_current_v6",
