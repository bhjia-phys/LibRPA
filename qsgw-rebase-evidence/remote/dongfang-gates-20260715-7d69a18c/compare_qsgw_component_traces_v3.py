...
def _parse_current_v5_header(
    text: str,
    label: str,
    *,
    expected_mixer: str,
    expected_mixing_beta: float,
    expected_hartree: bool,
) -> dict[str, str]:
    required_keys = CURRENT_V5_KEYS
    if expected_hartree:
        required_keys = required_keys | {
            "hartree_coulomb", "hartree_normalization"
        }
    values = _parse_header_values(text, label, required_keys)
    expected = {
        ...
        "hartree": "delta_density" if expected_hartree else "disabled_stage1",
        "band": "disabled_stage1",
        "qsgw_mixer": expected_mixer,
    }
    if expected_hartree:
        expected.update({
    ...
    if expected_mixer == "linear" and not math.isclose(
        beta, expected_mixing_beta, rel_tol=0.0, abs_tol=1.0e-15
    ):
        raise ValueError(
            f"{label}: qsgw_mixing_beta {beta} does not match "
            f"expected {expected_mixing_beta}"
        )
    return values
...
def _validate_legacy_current_contract(
    ...
    expected_mixing_beta: float,
    expected_current_mixer: str,
    expected_hartree: bool,
    expected_use_shrink_abfs: bool,
) -> dict[str, object]:
    ...
    old_expected = {
        ...
        "qsgw_mixer": "linear",
        "qsgw_mixing_beta": str(expected_mixing_beta),
        "starting_vxc": "dft_only",
        "vxc_basis": "fixed_state",
        "qsgw_update_hartree": "1" if expected_hartree else "0",
        ...
        "use_shrink_abfs": "1" if expected_use_shrink_abfs else "0",
        "use_fullcoul_exx": (
    ...
    current_headers = [
        (
            label,
            _parse_current_v5_header(
                text,
                label,
                expected_mixer=expected_current_mixer,
                expected_mixing_beta=expected_mixing_beta,
                expected_hartree=expected_hartree,
            ),
        )
    ...
    beta_label = format(expected_mixing_beta, ".15g")
    direct_update_mapping = (
        f"legacy_linear_beta_{beta_label}_to_"
        f"current_linear_beta_{beta_label}"
        if expected_current_mixer == "linear"
        else "legacy_linear_beta_1_to_current_none"
    )
    return {
        "passed": True,
        "old_version": 4,
        ...
        "direct_update_mapping": direct_update_mapping,
        "mixing_beta": expected_mixing_beta,
        ...
        "hartree_enabled": expected_hartree,
        "legacy_use_shrink_abfs": expected_use_shrink_abfs,
...
def _selected_common_rows(
    ...
    if expected_hartree:
        common_components.add("delta_vh")
    selected = {
        key: value
    ...
    for iteration in sorted(iterations):
        ...
        if expected_hartree and iteration > 0:
            required.add("delta_vh")
...
def compare_legacy_v4_current_v5(
    *,
    ...
    expected_mixing_beta: float = 1.0,
    expected_current_mixer: str = "none",
    expected_hartree: bool = False,
    expected_use_shrink_abfs: bool = False,
) -> dict[str, object]:
    if channel != 0:
    ...
    if not isinstance(expected_hartree, bool):
        raise ValueError("expected_hartree must be a bool")
    if not isinstance(expected_use_shrink_abfs, bool):
        raise ValueError("expected_use_shrink_abfs must be a bool")
    if (
        not math.isfinite(expected_mixing_beta)
        or expected_mixing_beta <= 0.0
        or expected_mixing_beta > 1.0
    ):
        raise ValueError("expected mixing beta must be in (0, 1]")
    if expected_current_mixer not in {"none", "linear"}:
        raise ValueError("expected current mixer must be none or linear")
    if expected_current_mixer == "none" and not math.isclose(
        expected_mixing_beta, 1.0, rel_tol=0.0, abs_tol=1.0e-15
    ):
        raise ValueError(
            "current mixer none is equivalent only to legacy linear beta 1"
        )
    ...
    contract = _validate_legacy_current_contract(
        ...
        expected_mixing_beta,
        expected_current_mixer,
        expected_hartree,
        expected_use_shrink_abfs,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--expected-mixing-beta", type=float, default=1.0
    )
    parser.add_argument(
    ...
    try:
        if args.contract_mode == "legacy_v4_to_current_v5":
            ...
            report = compare_legacy_v4_current_v5(
                ...
                expected_mixing_beta=args.expected_mixing_beta,
                expected_current_mixer=args.expected_current_mixer,
                expected_hartree=args.expected_hartree == "1",
                expected_use_shrink_abfs=(
                    args.expected_use_shrink_abfs == "1"
                ),
            )
        else:
