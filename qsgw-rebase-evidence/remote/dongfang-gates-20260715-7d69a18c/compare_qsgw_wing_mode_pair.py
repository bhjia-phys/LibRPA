...
QSGW_COMPLETED = re.compile(
    r"(?m)^QSGW completed iterations:\s*([0-9]+)\s*$"
)
...
def _completed_iterations(text: str, label: str) -> int:
    values = QSGW_COMPLETED.findall(text)
    if len(values) != 1:
        raise ValueError(
            f"{label} stdout must report QSGW completed iterations once"
        )
    completed = int(values[0])
    if completed <= 0:
        raise ValueError(f"{label} QSGW completed iteration count is invalid")
    return completed
...
def compare(
    ...
    *,
    ...
    head_completed = _completed_iterations(head_only_stdout, "head-only")
    wing_completed = _completed_iterations(
        head_plus_wing_stdout, "head-plus-wing"
    )
    if head_completed != wing_completed:
        raise ValueError("controlled pair completed different QSGW iterations")
    ...
    passed = bool(
        ...
        and wing_success_count == wing_completed
    ...
    return {
        ...
        "qsgw_completed_iterations": wing_completed,
