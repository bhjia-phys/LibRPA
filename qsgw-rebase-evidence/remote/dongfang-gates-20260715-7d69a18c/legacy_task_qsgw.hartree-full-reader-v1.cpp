...
void write_iteration_history_file(const std::string &path)
{
    ...
    for (size_t i = 0; i < iteration_numbers.size(); ++i)
    {
        file << std::scientific << std::setprecision(15)
             << iteration_numbers[i] << " " << homo_values[i] << " " << lumo_values[i] << " "
...
using OracleSpinKMatrixMap = std::map<int, std::map<int, Matz>>;
using OracleSpinKFrequencyMatrixMap =
    std::map<int, std::map<int, std::map<double, Matz>>>;

bool oracle_env_bool(const char *name, const bool default_value)
{
...
OracleSpinKMatrixMap deep_copy_spin_k_map(const OracleSpinKMatrixMap &input)
{
    OracleSpinKMatrixMap result;
    for (const auto &spin_entry : input)
    {
        for (const auto &kpoint_entry : spin_entry.second)
        {
            result[spin_entry.first][kpoint_entry.first] =
                kpoint_entry.second.copy();
        }
    }
    return result;
}

OracleSpinKMatrixMap linear_mix_spin_k_map(
    const OracleSpinKMatrixMap &input,
    const OracleSpinKMatrixMap &output,
    const double beta)
{
    OracleSpinKMatrixMap result = deep_copy_spin_k_map(input);
    for (auto &spin_entry : result)
    {
        for (auto &kpoint_entry : spin_entry.second)
        {
            const Matz &target =
                output.at(spin_entry.first).at(kpoint_entry.first);
            Matz &mixed = kpoint_entry.second;
            if (mixed.nr() != target.nr() || mixed.nc() != target.nc())
            {
                throw std::runtime_error(
                    "Legacy Scheme-A mixer matrix shapes do not match");
            }
            for (int row = 0; row < mixed.nr(); ++row)
            {
                for (int column = 0; column < mixed.nc(); ++column)
                {
                    mixed(row, column) +=
                        beta * (target(row, column) - mixed(row, column));
                }
            }
        }
    }
...
void write_oracle_rows(
    std::ostream &output, const int iteration, const std::string &component,
    const int spin, const int kpoint, const int frequency_index,
    const double frequency, const Matz &matrix)
{
...
void write_oracle_rows(
    std::ostream &output, const int iteration, const std::string &component,
    const int spin, const int kpoint, const ComplexMatrix &matrix)
{
    output << std::scientific << std::setprecision(15);
    for (int row = 0; row < matrix.nr; ++row)
    {
...
void write_oracle_matrix_component(
    std::ostream &output, const int iteration, const std::string &component,
    const OracleSpinKMatrixMap &matrices)
{
    for (const auto &spin_entry : matrices)
    {
        for (const auto &kpoint_entry : spin_entry.second)
        {
            write_oracle_rows(
                output, iteration, component, spin_entry.first,
                kpoint_entry.first, -1, 0.0, kpoint_entry.second);
        }
    }
}

void write_oracle_occupations(
    std::ostream &output, const int iteration, const MeanField &mf)
{
    for (int spin = 0; spin < mf.get_n_spins(); ++spin)
    {
        for (int kpoint = 0; kpoint < mf.get_n_kpoints(); ++kpoint)
        {
            Matz occupations(1, mf.get_n_bands(), MAJOR::COL);
            for (int band = 0; band < mf.get_n_bands(); ++band)
            {
                occupations(0, band) =
                    mf.get_weight()[spin](kpoint, band);
            }
            write_oracle_rows(
                output, iteration, "occupation", spin, kpoint,
                -1, 0.0, occupations);
        }
    }
}

void write_oracle_scalar(
    std::ostream &output, const int iteration,
    const std::string &component, const double value)
{
    if (!std::isfinite(value))
    {
        throw std::invalid_argument(
            "Legacy Scheme-A scalar trace value is non-finite");
    }
    Matz scalar(1, 1, MAJOR::COL);
    scalar(0, 0) = value;
    write_oracle_rows(
        output, iteration, component, 0, 0, -1, 0.0, scalar);
}

void write_oracle_frequency_component(
    std::ostream &output, const int iteration, const std::string &component,
    const OracleSpinKFrequencyMatrixMap &matrices)
{
    for (const auto &spin_entry : matrices)
    {
        for (const auto &kpoint_entry : spin_entry.second)
        {
            ...
            for (const auto &frequency_entry : kpoint_entry.second)
            {
                write_oracle_rows(
                    output, iteration, component, spin_entry.first,
                    kpoint_entry.first, frequency_index,
                    frequency_entry.first, frequency_entry.second);
                ++frequency_index;
            }
        }
    }
...
void write_oracle_wavefunctions(
    std::ostream &output, const int iteration, const MeanField &mf,
    const bool fixed_reference)
{
    const auto &wavefunctions = fixed_reference
        ? mf.get_eigenvectors0() : mf.get_eigenvectors();
    for (int spin = 0; spin < mf.get_n_spins(); ++spin)
    {
        for (int spinor = 0; spinor < mf.get_n_soc(); ++spinor)
        {
            const std::string component =
                "wfc_spinor" + std::to_string(spinor);
            for (int kpoint = 0; kpoint < mf.get_n_kpoints(); ++kpoint)
            {
                write_oracle_rows(
                    output, iteration, component, spin, kpoint,
                    wavefunctions.at(spin).at(spinor).at(kpoint));
            }
        }
    }
}

void write_oracle_velocity(
    std::ostream &output, const int iteration, const MeanField &mf,
    const bool fixed_reference)
{
    static const char *component_names[3] = {
        "velocity_x", "velocity_y", "velocity_z"};
    const auto &velocity = fixed_reference
        ? mf.get_velocity0() : mf.get_velocity();
    if (velocity.empty())
    {
        return;
    }
    for (int spin = 0; spin < mf.get_n_spins(); ++spin)
    {
        for (int kpoint = 0; kpoint < mf.get_n_kpoints(); ++kpoint)
        {
            if (velocity.at(spin).at(kpoint).size() != 3)
            {
                throw std::runtime_error(
                    "Legacy Scheme-A velocity trace requires three components");
            }
            for (int direction = 0; direction < 3; ++direction)
            {
                write_oracle_rows(
                    output, iteration, component_names[direction], spin,
                    kpoint, velocity.at(spin).at(kpoint).at(direction));
            }
        }
    }
}

void write_oracle_contract_header(
    std::ostream &output, const double beta, const int max_iterations,
    const bool update_hartree)
{
    output << "# qsgw_contract_version 4\n"
           << "# oracle_kind legacy_scheme_a\n"
           << "# oracle_source_commit 7a7ff17f\n"
           << "# task qsgw\n"
           << "# fixed_basis immutable_reference\n"
           << "# qsgw_mixer linear\n"
           << "# qsgw_mixing_beta " << std::setprecision(17) << beta << "\n"
           << "# qsgw_min_iter " << max_iterations << "\n"
           << "# qsgw_max_iter " << max_iterations << "\n"
           << "# starting_vxc dft_only\n"
           << "# vxc_basis fixed_state\n"
           << "# qsgw_update_hartree " << (update_hartree ? 1 : 0) << "\n"
           << "# qsgw_hartree_coulomb truncated\n"
           << "# qsgw_hartree_normalization legacy_extra_inverse_nk\n"
           << "# use_symmetry_gw "
           << (Params::use_abacus_gw_symmetry ? 1 : 0) << "\n"
           << "# use_symmetry_exx "
           << (Params::use_abacus_exx_symmetry ? 1 : 0) << "\n"
           << "# replace_w_head " << (Params::replace_w_head ? 1 : 0) << "\n"
           << "# option_dielect_func " << Params::option_dielect_func << "\n"
           << "# nfreq " << Params::nfreq << "\n"
           << "# n_params_anacon " << Params::n_params_anacon << "\n"
           << "# n_params_anacon_resample -1\n"
           << "# anacon_nfreq -1\n"
           << "# anacon_tfgrids_type -101\n"
           << "# use_shrink_abfs " << (Params::use_shrink_abfs ? 1 : 0) << "\n"
           << "# use_fullcoul_exx " << (Params::use_fullcoul_exx ? 1 : 0) << "\n"
           << "# use_fullcoul_eps 1\n"
           << "# use_fullcoul_wc " << (Params::use_fullcoul_wc ? 1 : 0) << "\n"
           << "# constants_choice internal\n"
           << "# ac_policy direct_pade\n"
           << "# iter channel component spin kpoint frequency_index "
...
void task_qsgw(std::map<Vector3_Order<double>, ComplexMatrix> &sinvS)
{
    ...
    for (int ispin = 0; ispin < meanfield.get_n_spins(); ++ispin)
    {
        for (int ikpt = 0; ikpt < meanfield.get_n_kpoints(); ++ikpt)
        {
            ...
            if (!s_file_found) {
                ...
                std::string sNewFilePath = input_dir + "/" + oss_s_new.str();
            ...
            if (vxc_file.good()) {
                ...
                std::string vxcNewFilePath = input_dir + "/" + oss_vxc_new.str();
                ...
                if (vxc_new_format_file.good()) {
                    try {
                        ...
                        {
                            throw std::runtime_error(
                                "Legacy Scheme-A ABACUS Vxc supports only n_spinor=1");
                        }
                        if (matrix_size != n_aos)
                        {
                            throw std::runtime_error(
                                "Legacy Scheme-A ABACUS Vxc dimension does not match n_aos");
                        }
                        const Matz vxc_nao_ha = 0.5 * vxc_matrix;
                        vxc0[ispin][ikpt] =
                            conj(wfc1) * vxc_nao_ha * transpose(wfc1);
                        vxc_file_found = true;
                    } catch (const std::exception& e) {
                        all_files_processed_successfully = false;
            ...
            // Scheme A uses exactly the DFT Vxc already contained in H_KS0.
            // The producer HF file is ignored; Sigma_x comes from the legacy
            // per-iteration EXX kernel.
            if (vxc0[ispin][ikpt].nr() != n_bands ||
                vxc0[ispin][ikpt].nc() != n_bands)
            {
                throw std::runtime_error(
                    "Legacy Scheme-A Vxc is not in the fixed state basis");
            }
    ...
    QsgwZeroTemperatureOccupationResult initial_occupations;
    {
        const MeanField producer_reference = meanfield;
        total_electrons = qsgw_physical_electron_count(
            producer_reference, qsgw_kpoint_weights);
        initial_occupations = update_qsgw_zero_temperature_occupations(
            meanfield, producer_reference, qsgw_kpoint_weights,
            total_electrons);
    }
    const MeanField reference_meanfield = meanfield;
    efermi = initial_occupations.chemical_potential;
    homo = initial_occupations.vbm;
    lumo = initial_occupations.cbm;
    printf("%5s\n", "Total_electrons");
    printf("%5f\n", total_electrons);
    ...
    const bool oracle_trace_enabled =
    ...
    {
        throw std::runtime_error(
            "Legacy Scheme-A mixing beta must be in (0, 1]");
    }
    if (Params::qsgw_restart)
    {
        throw std::runtime_error(
            "Legacy Scheme-A oracle requires a continuous run from iteration zero");
    }
    ...
    OracleSpinKMatrixMap previous_mixed_h0 =
        deep_copy_spin_k_map(H_KS0);
    ...
    if (mpi_comm_global_h.is_root())
    {
        ...
        if (oracle_trace_enabled)
        {
            const std::string trace_path =
                Params::output_dir + "qsgw_oracle_matrices.dat";
            oracle_trace.open(trace_path, std::ios::trunc);
            if (!oracle_trace.good())
            {
                throw std::runtime_error(
                    "Failed to open legacy Scheme-A trace: " + trace_path);
            }
            write_oracle_contract_header(
                oracle_trace, mixing_beta, max_iterations,
                oracle_update_hartree);
            write_oracle_matrix_component(
                oracle_trace, 0, "h0", H_KS0);
            write_oracle_matrix_component(
                oracle_trace, 0, "vxc_dft", vxc0);
            write_oracle_wavefunctions(
                oracle_trace, 0, meanfield, true);
            write_oracle_velocity(
                oracle_trace, 0, meanfield, true);
            write_oracle_occupations(
                oracle_trace, 0, meanfield);
            write_oracle_scalar(
                oracle_trace, 0, "fermi_energy_ha",
                initial_occupations.chemical_potential);
            write_oracle_scalar(
                oracle_trace, 0, "electron_count",
                initial_occupations.electron_count);
            write_oracle_scalar(
                oracle_trace, 0, "gap_ha", initial_occupations.gap);
            oracle_trace.flush();
        }
        ...
        if (Params::qsgw_restart)
        {
            ...
            diagonalize_and_store_fixed_basis(meanfield, checkpoint.H0_GW_all, n_spins, n_kpoints,
                                              n_bands);
            const auto checkpoint_occupations =
                update_qsgw_zero_temperature_occupations(
                    meanfield, reference_meanfield,
                    qsgw_kpoint_weights, total_electrons);
            homo = checkpoint_occupations.vbm;
            lumo = checkpoint_occupations.cbm;
    ...
    while (!converged && iteration < max_iterations)
    {
        ...
        if (oracle_update_hartree)
        {
            ...
            for (int ispin = 0; ispin < meanfield.get_n_spins(); ++ispin)
            {
                for (int ikpt = 0; ikpt < meanfield.get_n_kpoints(); ++ikpt)
                {
                    Hartree_i[ispin][ikpt] =
                        Matz(n_bands, n_bands, MAJOR::COL);
                    if (iteration == 1)
                    {
                        Hartree_0[ispin][ikpt] =
                            Matz(n_bands, n_bands, MAJOR::COL);
                    }
                    ...
                    for (int i = 0; i < n_bands; ++i)
                    {
                        for (int j = 0; j < n_bands; ++j)
                        {
                            Hartree_i[ispin][ikpt](i, j) =
                                Hartree.Hartree_is_ik_KS[ispin][ikpt](i, j);
                            if (iteration == 1)
                            {
                                Hartree_0[ispin][ikpt](i, j) =
                                    Hartree_i[ispin][ikpt](i, j);
                            }
                            else
                            {
                                Hartree_i_delta[ispin][ikpt](i, j) =
                                    Hartree_i[ispin][ikpt](i, j) -
                                    Hartree_0[ispin][ikpt](i, j);
                            }
                        }
        ...
        if (all_files_processed_successfully)
        {
            ...
            if (mpi_comm_global_h.is_root())
            {
                if (oracle_trace_enabled)
                {
                    write_oracle_frequency_component(
                        oracle_trace, iteration, "sigma_c_iw",
                        s_g0w0.sigc_is_ik_f_KS);
                }
                // 遍历自旋、k点和能带状态
                for (int i_spin = 0; i_spin < n_spins; i_spin++)
                {
                    for (int i_kpoint = 0; i_kpoint < n_kpoints; i_kpoint++)
                    {
                        ...
                        for (int i_state_row = 0; i_state_row < n_bands; i_state_row++)
                        {
                                 ...
                                 i_state_col++)
                            {
                                ...
                                {
                                    sigc_mn.push_back(
                                        sigc_sk.at(freq)(i_state_row, i_state_col));
                                }
                                ...
                                LIBRPA::AnalyContPade pade(
                                    Params::n_params_anacon, imagfreqs, sigc_mn);
                                const cplxdb result = pade.get(energy0 - efermi);
                                const cplxdb result1 = pade.get(0.0);
                        ...
                        if (oracle_update_hartree && iteration > 1) {
                ...
                {
                    write_oracle_matrix_component(
                        oracle_trace, iteration, "exx",
                        exx.exx_is_ik_KS);
                    write_oracle_matrix_component(
                        oracle_trace, iteration, "vc", Vc_GW_pure);
                    if (oracle_update_hartree)
                    {
                        write_oracle_matrix_component(
                            oracle_trace, iteration, "delta_vh",
                            Hartree_i_delta);
                    }
                }
                ...
                {
                    write_oracle_matrix_component(
                        oracle_trace, iteration, "raw_h", H0_GW_all);
                }
                H0_GW_all = linear_mix_spin_k_map(
                    previous_mixed_h0, H0_GW_all, mixing_beta);
                previous_mixed_h0 = deep_copy_spin_k_map(H0_GW_all);
                if (oracle_trace_enabled)
                {
                    write_oracle_matrix_component(
                        oracle_trace, iteration, "mixed_h", H0_GW_all);
                }

                OracleSpinKMatrixMap rotation_u;
                diagonalize_and_store_fixed_basis(
                    meanfield, H0_GW_all, n_spins, n_kpoints, n_bands,
                    &rotation_u);
                if (oracle_trace_enabled)
                {
                    write_oracle_matrix_component(
                        oracle_trace, iteration, "rotation_u", rotation_u);
                    write_oracle_wavefunctions(
                        oracle_trace, iteration, meanfield, false);
                    write_oracle_velocity(
                        oracle_trace, iteration, meanfield, false);
                    oracle_trace.flush();
                }
                ...
                const auto occupation_result =
                    update_qsgw_zero_temperature_occupations(
                        meanfield, reference_meanfield,
                        qsgw_kpoint_weights, total_electrons);
                efermi = occupation_result.chemical_potential;
                printf("%5s\n", "efermi_qsgw");
                printf("%5f\n", efermi);
                ...
                {
                    write_oracle_occupations(
                        oracle_trace, iteration, meanfield);
                    write_oracle_scalar(
                        oracle_trace, iteration, "fermi_energy_ha",
                        occupation_result.chemical_potential);
                    write_oracle_scalar(
                        oracle_trace, iteration, "electron_count",
                        occupation_result.electron_count);
                    write_oracle_scalar(
                        oracle_trace, iteration, "gap_ha",
                        occupation_result.gap);
                    oracle_trace.flush();
                }
                ...
                if (mpi_comm_global_h.is_root())
                {
                    ...
                    const bool tolerance_reached =
                        eigdiff_for_conv < eigenvalue_diff_tolerance;
                    converged = tolerance_reached &&
                                iteration >= max_iterations;
                    if (tolerance_reached && iteration < max_iterations)
                    {
                        std::cout << "Scheme-A tolerance reached at iteration "
                                  << iteration
                                  << "; continuing to the required iteration "
                    ...
                    {
                        write_qsgw_checkpoint(
                            checkpoint_save_root, iteration, H0_GW_all,
                            efermi,
                            oracle_update_hartree ? &Hartree_0 : nullptr,
                            nullptr);
                    }
                }
            }
