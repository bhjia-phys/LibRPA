...
from validate_qsgw_trace_closure_v5 import (  # noqa: E402
    TraceClosureError,
...
def _rows(
    ...
    for row in range(matrix.shape[0]):
        for column in range(matrix.shape[1]):
            ...
            result.append(
                f"{iteration} {channel} {component} 0 0 -1 0.0 {row} {column} "
...
def _current_band_trace(*, band_mode: str = "fixed_reference_operator_fourier_live") -> str:
    raw1 = _materialize_upper_triangle_hermitian(H0 - VXC + EXX + VC)
    raw2 = _materialize_upper_triangle_hermitian(H0 - VXC + EXX + VC)
    mixed1 = (1.0 - BETA) * H0 + BETA * raw1
    mixed2 = (1.0 - BETA) * mixed1 + BETA * raw2
    ...
    for component, matrix in (("h0", H0), ("vxc_dft", VXC)):
        lines.extend(_rows(0, component, matrix, channel=0))
        lines.extend(_rows(0, component, matrix, channel=1))
    for component, matrix in (
        ("exx", EXX),
        ("vc", VC),
        ("raw_h", raw1),
        ...
        lines.extend(_rows(1, component, matrix, channel=0))
        lines.extend(_rows(1, component, matrix, channel=1))
    for component, matrix in (
        ("exx", EXX),
        ("vc", VC),
        ("raw_h", raw2),
        ...
        lines.extend(_rows(2, component, matrix, channel=0))
        lines.extend(_rows(2, component, matrix, channel=1))
    return "\n".join(lines) + "\n"
...
class CurrentBandClosureTests(unittest.TestCase):
    def test_band_trace_grid_channel_passes(self) -> None:
        report = validate_trace_text(
            _current_band_trace(), (0, 1, 2), channel=0
        )
        self.assertTrue(report["passed"])

    def test_band_trace_band_channel_passes(self) -> None:
        report = validate_trace_text(
            _current_band_trace(), (0, 1, 2), channel=1
        )
        self.assertTrue(report["passed"])

    def test_band_channel_on_grid_only_trace_is_rejected(self) -> None:
        with self.assertRaisesRegex(TraceClosureError, "qsgw_band"):
            validate_trace_text(
                _current_hartree_trace(), (0, 1, 2), channel=1
            )

    def test_invalid_band_mode_is_rejected(self) -> None:
        with self.assertRaisesRegex(TraceClosureError, "unsupported band mode"):
            validate_trace_text(
                _current_band_trace(band_mode="fourier_state_basis"),
                (0, 1, 2),
                channel=0,
            )
