...
Revision v5 additionally accepts current (contract version 5) traces with
the operator-Fourier band channel enabled
(``band fixed_reference_operator_fourier_live``), mapping them to
``task=qsgw_band`` so channel-1 validation is possible. v4 accepted the
Hartree delta-density mode. The numerical closure, Hermiticity, Hartree
and mixing checks are unchanged.
"""
...
def _parse_stage_one_contract(text: str) -> dict[str, str]:
    ...
    band_mode = contract["band"]
    if band_mode not in {
        "disabled_stage1",
        "fixed_reference_operator_fourier_live",
    }:
        raise TraceClosureError(
            "invalid QSGW stage-one trace contract: unsupported band mode "
            f"{band_mode!r}"
        )
    band_enabled = band_mode == "fixed_reference_operator_fourier_live"
    ...
    return {
        **contract,
        "task": "qsgw_band" if band_enabled else "qsgw",
