...
dataset directory containing links or copies of the physical input files.
"""
...
def build_contract(
    ...
    if update_hartree:
        ri_files = sorted(
            (require_file(path) for path in dataset.iterdir() if path.name.startswith("Cs_data")),
            key=lambda path: path.name,
        )
        coulomb_prefix = "coulomb_mat" if hartree_coulomb == "full" else "coulomb_cut"
        ...
        role_files.append(
            ("hartree_aux_basis", require_file(dataset / "basis_aux_out"))
        )
