...
run_root=$base/librpa-qsgw-gate-a0-exact847-exx-band-isolation-20260720-v2
...
# exx_band requires the legacy scalar vxc_out before entering its band path.
# These zero values are used only for the unused SCF-grid report; band output
# reads the real band_vxck*_nao.txt matrices from the immutable dataset.
{
  printf '8 1 44\n'
  for _ in $(seq 1 $((8 * 44))); do
    printf '0.0 0.0\n'
  done
} >"$run_root/work/vxc_out"
...
gate=gate_a0_exact847_standalone_exx_band_isolation_v2
...
band_vxc_source=immutable_band_vxck_nao_files
mpi_ranks=$mpi_ranks
omp_threads=$omp_threads
started_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)
EOF

stage=analyzer_tests
(
  cd "$run_root/tools"
...
! grep -Fq 'task failed' "$run_root/work/librpa.stdout"
...
echo GATE_A0_EXACT847_EXX_BAND_ISOLATION_V2=COMPLETE
