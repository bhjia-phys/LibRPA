...
run_root=$base/librpa-qsgw-gate-a1-exact847-scheme-a-legacy-symmetry-nomix-miniter2-20260720-v4
...
legacy_build=/tmp/librpa-qsgw-gate-a0-legacy-exact847-band0-scheme-a-20260720-v12/build
...
python3 - "$dataset" "$run_root/band-occupation-serialization-audit.json" <<'PY'
import json
import pathlib
import sys
...
files = sorted(dataset.glob("band_KS_eigenvalue_k_*.txt"))
assert len(files) == 201, len(files)
capacity = 2.0 / len(files)
occupied = []
occupied_per_k = []
per_k_sums = []
for path in files:
    values = [float(line.split()[2]) for line in path.read_text().splitlines() if line.split()]
    positive = [value for value in values if value > 0.0]
    occupied.extend(positive)
    occupied_per_k.append(len(positive))
    per_k_sums.append(sum(values))
total = sum(occupied)
report = {
    "n_kpoints": len(files),
    "capacity_exact": capacity,
    "occupied_state_count": len(occupied),
    "occupied_per_k_unique": sorted(set(occupied_per_k)),
    "stored_occupied_unique": sorted(set(occupied)),
    "max_per_state_excess": max(value - capacity for value in occupied),
    "total_stored_electrons": total,
    "total_excess_from_8": total - 8.0,
    "per_k_sum_min": min(per_k_sums),
    "per_k_sum_max": max(per_k_sums),
    "default_1e_12_capacity_check_passes": all(
        value <= capacity + 1.0e-12 for value in occupied
    ),
    "band_1e_8_capacity_check_passes": all(
        value <= capacity + 1.0e-8 for value in occupied
    ),
    "band_1e_8_total_check_passes": abs(total - 8.0) <= 1.0e-8,
}
assert report["occupied_per_k_unique"] == [4], report
assert report["occupied_state_count"] == 804, report
assert not report["default_1e_12_capacity_check_passes"], report
assert report["band_1e_8_capacity_check_passes"], report
assert report["band_1e_8_total_check_passes"], report
report["passed"] = True
...
gate=gate_a1_exact847_scheme_a_legacy_symmetry_nomix_miniter2_v4
...
test "$(grep -Fc 'QSGW band0: grid updated electrons =' "$legacy/librpa.stdout")" -eq 2
test "$(grep -Fc 'QSGW band0: band updated electrons =' "$legacy/librpa.stdout")" -eq 2
test "$(grep -Fc 'QSGW band0: band reference electrons =' "$legacy/librpa.stdout")" -eq 1
...
grid_updated = [
    ...
    for line in lines
    if "QSGW band0: grid updated electrons =" in line
]
band_updated = [
    float(line.split("=")[1].strip())
    for line in lines
    if "QSGW band0: band updated electrons =" in line
]
assert len(grid) == 1, grid
assert len(band) == 2, band
assert len(grid_updated) == 2, grid_updated
assert len(band_updated) == 2, band_updated
assert all(
    abs(value - 8.0) <= 1.0e-10
    for value in grid + band + grid_updated + band_updated
), (grid, band, grid_updated, band_updated)
electron_report = {
    "target_electrons": 8.0,
    "tolerance": 1.0e-10,
    "grid_reference_values": grid,
    "band_iteration_values": band,
    "grid_updated_values": grid_updated,
    "band_updated_values": band_updated,
...
echo GATE_A1_EXACT847_SCHEME_A_LEGACY_SYMMETRY_NOMIX_MINITER2_V4=PASS
cat "$run_root/band-occupation-serialization-audit.json"
