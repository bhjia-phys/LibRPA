...
SCHEMA = "librpa-qsgw-mixing-frontier-audit-v2"
    ...
    for kpoint in range(n_kpoints):
        for spin in range(n_spins):
            ...
            for band in range(n_bands):
                ...
                unit_column_tolerance_ev = 5.0e-6 * max(1.0, abs(energy_ev))
                if abs(energy_ev - energy_ha * HA2EV) > unit_column_tolerance_ev:
