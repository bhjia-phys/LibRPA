...
run_root=/home/bhj/ai-runs/librpa-qsgw-gate-a0-legacy-847reader-ibzocc-smoke-20260720-v3
build_evidence=/home/bhj/ai-runs/librpa-qsgw-gate-a0-legacy-847reader-ibzocc-build-20260720-v7
...
gate=gate_a0_legacy_847reader_ibzocc_smoke_v3
...
if grep -Fq 'Cannot find overlap matrix file' librpa.stdout librpa.stderr; then
  echo 'legacy oracle did not consume all overlap matrices' >&2
  exit 1
fi
if grep -Fq 'occupation exceeds storage capacity' librpa.stdout librpa.stderr; then
  echo 'legacy oracle rejected geometrically weighted IBZ occupations' >&2
  exit 1
fi
...
echo GATE_A0_LEGACY_847READER_IBZOCC_SMOKE_V3=PASS
exit 0
