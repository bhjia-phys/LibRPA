...
run_root=/home/bhj/ai-runs/librpa-qsgw-gate-a0-legacy-847reader-ibzocc-build-20260720-v7
legacy_source=/tmp/librpa-qsgw-gate-a0-legacy-e08f4a13-847reader-ibzocc-20260720-v7
...
legacy_patch=/tmp/legacy_oracle_847_reader_ibz_occupation_restore.patch
...
expected_legacy_patch_sha=d34bf83fb394f1f58c86b2c387b8e1fbba8a69bbbc783ebc770d1c2de69903ed
...
test "$(sha256sum "$legacy_patch" | awk '{print $1}')" = \
  "$expected_legacy_patch_sha"
...
git -C "$legacy_source" apply --check "$legacy_patch"
git -C "$legacy_source" apply "$legacy_patch"
...
git -C "$legacy_source" diff -- \
  ...
  >"$run_root/legacy-oracle-restore.patch"
test -s "$run_root/legacy-oracle-restore.patch"
cp "$legacy_patch" "$run_root/legacy-oracle-input.patch"
...
grep -Fq 'capacity_factor *' \
  "$legacy_source/qsgw/fermi_energy_occupation.cpp"
grep -Fq 'electrons += stored;' \
  "$legacy_source/qsgw/fermi_energy_occupation.cpp"
grep -Fq 'result.electron_count += stored;' \
  "$legacy_source/qsgw/fermi_energy_occupation.cpp"
if grep -Fq 'stored * active_kpoint_count' \
  "$legacy_source/qsgw/fermi_energy_occupation.cpp"; then
  echo 'legacy oracle still double-applies IBZ k-point weights' >&2
  exit 1
fi
if grep -Fq 'std::vector<size_t> shrinked_mu;' \
  "$legacy_source/driver/read_data.cpp"; then
  echo 'legacy shrink reader still uses shard-local inference' >&2
  exit 1
fi
...
gate=gate_a0_executable_freeze_v7
acceptance=true
legacy_role=merge_before_qsgw_oracle_with_historical_reader_and_ibz_occupation_restore
legacy_commit=$legacy_commit
historical_behavior_commit=$historical_behavior_commit
legacy_patch_sha256=$expected_legacy_patch_sha
legacy_patch_scope=driver/read_data.cpp_qsgw/fermi_energy_occupation.cpp_src/librpa.cpp_only_oracle_build
...
echo GATE_A0_LEGACY_847READER_IBZOCC_BUILD_V7=PASS
