...
run_root=$base/librpa-qsgw-gate-a1-exact847-candidate-symmetry-nomix-miniter2-20260720-v2
baseline=$base/librpa-qsgw-gate-a0-historical-toolchain-attestation-20260720-v1
fish_grid_run=$base/librpa-qsgw-gate-a0-legacy-exact847-band0-iter1-20260720-v1
fish_band_comparison=$base/librpa-qsgw-gate-a0-legacy-exact847-band0-comparison-20260720-v1
...
expected_baseline_output_sha=4d8660d55c93029bc1212d1f4dd6bd0d9f65dac35c818103c7ddf26ae66a196d
expected_baseline_provenance_sha=a7203122bb254ccfe84fd25c746bc129810760b8e214e2e16c4f08b56efe67cd
expected_fish_grid_output_sha=761e3cd1df5bbfc3a430f78bb55d0ce87b9fc7159ff2a4c90ef29c70a514557c
expected_fish_band_comparison_sha=e7f7b164241c5b16bf3cba764dd72e0e46add8ee90c783ce5923f225b667476c
...
test -e "$baseline/HISTORICAL_GRID_AND_BAND_PARITY_GREEN"
test -e "$baseline/LEGACY_RAW_H0_INVARIANT_GAP"
test ! -e "$baseline/FAILED"
test -e "$fish_grid_run/RUN_GREEN"
test ! -e "$fish_grid_run/FAILED"
test -e "$fish_band_comparison/FAILED"
...
test "$(sha256sum "$baseline/OUTPUT_SHA256SUMS.txt" | awk '{print $1}')" = \
  "$expected_baseline_output_sha"
test "$(sha256sum "$baseline/PROVENANCE.txt" | awk '{print $1}')" = \
  "$expected_baseline_provenance_sha"
test "$(sha256sum "$fish_grid_run/OUTPUT_SHA256SUMS.txt" | awk '{print $1}')" = \
  "$expected_fish_grid_output_sha"
test "$(sha256sum "$fish_band_comparison/native-comparison-v2.json" | awk '{print $1}')" = \
  "$expected_fish_band_comparison_sha"
...
python3 -c \
  'import json,sys; r=json.load(open(sys.argv[1])); assert r["historical_reproduction_passed"] is False; assert r["h0"]["parity_passed"] is True; assert r["sigcrf"]["passed"] is True; assert r["text_outputs"]["passed"] is False; assert r["legacy_oracle_invariant_gap"] is True; print("fish_grid_core_parity=true"); print("fish_band_difference=toolchain_classified")' \
  "$fish_band_comparison/native-comparison-v2.json" \
  >"$run_root-baseline-classification.tmp"
...
mv "$run_root-baseline-classification.tmp" \
  "$run_root/BASELINE_CLASSIFICATION.txt"
...
gate=gate_a1_exact847_candidate_symmetry_nomix_miniter2_v2
acceptance=pending_native_checkpoint_vs_candidate_trace_comparison
historical_toolchain_baseline=$baseline
historical_toolchain_baseline_output_manifest_sha256=$expected_baseline_output_sha
historical_toolchain_baseline_parity=grid_and_band_green
historical_legacy_raw_h0_invariant=known_gap_not_relaxed
fish_exact847_grid_run=$fish_grid_run
fish_exact847_grid_output_manifest_sha256=$expected_fish_grid_output_sha
fish_exact847_band_comparison=$fish_band_comparison
fish_exact847_band_difference=classified_as_toolchain_sensitive
...
echo GATE_A1_EXACT847_CANDIDATE_SYMMETRY_NOMIX_MINITER2_V2=PASS
