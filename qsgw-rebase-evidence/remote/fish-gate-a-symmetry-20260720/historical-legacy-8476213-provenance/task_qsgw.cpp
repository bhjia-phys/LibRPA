...
bool old_qsgw_dump_iter1_enabled()
{
    const char* env = std::getenv("QSGW_OLD_DUMP_ITER1");
    return env != nullptr && std::string(env) == "1";
}

int old_qsgw_dump_kpt_index(const int n_kpoints)
{
    const char* env = std::getenv("QSGW_DUMP_KPT");
    int kpt_1b = env == nullptr ? 1 : std::atoi(env);
    if (kpt_1b < 1) kpt_1b = 1;
    if (kpt_1b > n_kpoints) kpt_1b = n_kpoints;
    return kpt_1b - 1;
}

std::string old_qsgw_dump_dir()
{
    const char* env = std::getenv("QSGW_DUMP_DIR");
    std::string dir = env == nullptr ? Params::output_dir + "qsgw_dump_old/" : std::string(env);
...
void old_dump_matz_mm(const Matz& mat, const std::string& fn_base, const std::string& label)
{
    std::ofstream ofs(fn_base + ".mm");
    if (!ofs.good())
    {
        return;
    }
...
void task_qsgw(std::map<Vector3_Order<double>, ComplexMatrix> &sinvS)
{
    ...
    const bool old_dump_iter1 = old_qsgw_dump_iter1_enabled();
    const int old_dump_kpt = old_qsgw_dump_kpt_index(n_kpoints);
    const std::string old_dump_dir = old_qsgw_dump_dir();
    if (old_dump_iter1 && mpi_comm_global_h.is_root())
    {
        std::cout << "[OLD QSGW] iter-1 diagnostic dump enabled: " << old_dump_dir
                  << " dump_kpt=" << old_dump_kpt + 1 << std::endl;
    }
            ...
            if (mpi_comm_global_h.is_root())
            {
                ...
                for (int i_spin = 0; i_spin < n_spins; i_spin++)
                {
                    for (int i_kpoint = 0; i_kpoint < n_kpoints; i_kpoint++)
                    {
                        ...
                        if (old_dump_iter1 && iteration == 1 && i_spin == 0 &&
                            i_kpoint == old_dump_kpt && mpi_comm_global_h.is_root())
                        {
                            old_dump_matz_mm(Vc_all[i_spin][i_kpoint],
                                             old_dump_dir + "old_Vc_all_spin0_kpt0",
                                             "old Vc_all spin=0 selected kpt");
                            const auto& sigc_spin_k =
                                s_g0w0.sigc_is_ik_f_KS.at(i_spin).at(i_kpoint);
                            int ifreq_dump = 0;
                            for (const auto& freq_dump : chi0.tfg.get_freq_nodes())
                            {
                                std::ostringstream stem;
                                stem << old_dump_dir << "old_SigcKS_spin0_kpt0_ifreq_"
                                     << std::setw(3) << std::setfill('0') << ifreq_dump;
                                old_dump_matz_mm(sigc_spin_k.at(freq_dump), stem.str(),
                                                 "old SigcKS spin=0 selected kpt");
                                ++ifreq_dump;
                            }
                        }
                ...
                if (old_dump_iter1 && iteration == 1 && mpi_comm_global_h.is_root())
                {
                    old_dump_matz_mm(H_KS0.at(0).at(old_dump_kpt),
                                     old_dump_dir + "old_H_KS0_spin0_kpt0",
                                     "old H_KS0 spin=0 selected kpt");
                    old_dump_matz_mm(vxc0.at(0).at(old_dump_kpt),
                                     old_dump_dir + "old_vxc0_spin0_kpt0",
                                     "old vxc0 spin=0 selected kpt");
                    old_dump_matz_mm(exx.exx_is_ik_KS.at(0).at(old_dump_kpt),
                                     old_dump_dir + "old_Hexx_iter_spin0_kpt0",
                                     "old Hexx spin=0 selected kpt");
                    old_dump_matz_mm(H0_GW_all.at(0).at(old_dump_kpt),
                                     old_dump_dir + "old_H0_GW_all_spin0_kpt0",
                                     "old H0_GW_all spin=0 selected kpt");
                }
