...
run_root=/home/bhj/ai-runs/librpa-qsgw-gate-a0-legacy-symmetry-oracle-build-20260720-v10
...
legacy_source=/tmp/librpa-qsgw-gate-a0-legacy-e08f4a13-symmetry-oracle-20260720-v10
...
shrink_patch=/tmp/legacy_oracle_shrink_sinvs_restore_v1.patch
...
shrink_patch_sha=d35d4512da864303ed5b26f6a6d4d1cdd59446aef574ce5f5437fc09eed74a00
...
test "$(sha256sum "$shrink_patch" | awk '{print $1}')" = "$shrink_patch_sha"
...
rm -rf "$legacy_build"
git -C "$legacy_source" apply --check --ignore-space-change --ignore-whitespace \
  "$qlist_patch"
git -C "$legacy_source" apply --ignore-space-change --ignore-whitespace \
  "$qlist_patch"
git -C "$legacy_source" apply --check --ignore-space-change --ignore-whitespace \
  "$shrink_patch"
git -C "$legacy_source" apply --ignore-space-change --ignore-whitespace \
...
cp "$shrink_patch" "$run_root/shrink-sinvs-restore.patch"

grep -Fq 'vector<Vector3_Order<double>> qlist = klist;' \
  "$legacy_source/driver/task_qsgw.cpp"
grep -Fq 'read_shrink_sinvS(driver_params.input_dir, "shrink_sinvS_", sinvS);' \
  "$legacy_source/driver/task_qsgw.cpp"
...
gate=gate_a0_legacy_symmetry_oracle_build_v10
acceptance=true_oracle_harness_build
legacy_role=merge_before_qsgw_oracle_with_historical_reader_ibz_occupation_qlist_and_shrink_transform_restore
...
shrink_transform_patch_sha256=$shrink_patch_sha
...
echo GATE_A0_LEGACY_SYMMETRY_ORACLE_BUILD_V10=PASS
