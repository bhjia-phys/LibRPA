...
run_root=/home/bhj/ai-runs/librpa-qsgw-gate0-20260716T0840-a76fd826
...
expected_candidate=a76fd826eed3929185ea47cfcb992075a72f2b77
...
git -C "$candidate_src" diff \
  "$expected_upstream" "$expected_candidate" -- \
  "${protected_paths[@]}" >"$evidence/protected-shared-implementations.diff"
test ! -s "$evidence/protected-shared-implementations.diff"

git -C "$candidate_src" diff \
  "$expected_upstream" "$expected_candidate" -- \
  src/core/dielecmodel.h >"$evidence/approved-head-getter.diff"
getter_added=$(awk '/^\+[^+]/ {count++} END {print count+0}' \
  "$evidence/approved-head-getter.diff")
getter_deleted=$(awk '/^-[^-]/ {count++} END {print count+0}' \
  "$evidence/approved-head-getter.diff")
test "$getter_added" -eq 5
test "$getter_deleted" -eq 0
grep -Fq 'get_head_matrices() const noexcept' \
  "$evidence/approved-head-getter.diff"
grep -Fq 'return head;' "$evidence/approved-head-getter.diff"
...
cp "$0" "$evidence/run-fish-gate0-95c4-a76fd826.sh"
...

{
  ...
  printf 'protected_shared_implementation_diff=zero\n'
  printf 'approved_head_getter_added_lines=%s\n' "$getter_added"
  printf 'approved_head_getter_deleted_lines=%s\n' "$getter_deleted"
...
(
  ...
  sha256sum \
    ...
    evidence/upstream-source-status-after.txt \
    evidence/candidate-source-status-after.txt \
    evidence/protected-shared-implementations.diff \
    evidence/approved-head-getter.diff \
    ...
    evidence/run-fish-gate0-95c4-a76fd826.sh \
