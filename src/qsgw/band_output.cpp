    ...
    const double chemical_potential_ha)
{
    ...
    const double fallback_state_weight =
        2.0 / (occupation_scale * live_band.get_n_spinor());
    ...
    for (int kpoint = 0; kpoint < live_band.get_n_kpoints(); ++kpoint)
    {
        ...
        for (int band = 0; band < live_band.get_n_bands(); ++band)
        {
            ...
            state_weight = std::max(state_weight, weight);
        }
        if (state_weight <= 1.0e-14)
            state_weight = fallback_state_weight;
        ...
        for (int band = 0; band < live_band.get_n_bands(); ++band)
        {
            ...
            const double qsgw_occupation =
                (qsgw_energy <= chemical_potential_ha
                     ? state_weight
                     : 0.0) * occupation_scale;
