...
SpinKMatrixMap assemble_effective_hamiltonian(
    ...
    const SpinKMatrixMap* hartree_delta)
{
    ...
    if (hartree_delta != nullptr)
    {
        validate_map(*hartree_delta, &reference_hamiltonian,
                     "Hartree delta");
    }
    ...
    for (const auto& [spin, by_kpoint] : reference_hamiltonian)
    {
        for (const auto& [kpoint, reference] : by_kpoint)
        {
            ...
            const Matz* hartree = hartree_delta == nullptr
                                      ? nullptr
                                      : &hartree_delta->at(spin).at(kpoint);
