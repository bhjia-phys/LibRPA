...
bool is_finite_matrix(const matrix& value)
{
    for (int index = 0; index < value.size; ++index)
    {
        if (!std::isfinite(value.c[index]))
    ...
    std::string& failure_reason)
{
    ...
    matrix gram(history_size, history_size, true);
    ...
    for (int i = 0; i < history_size; ++i)
    {
        for (int j = i; j < history_size; ++j)
        {
            ...
            if (!std::isfinite(value))
            {
                ...
                failure_reason = "Pulay residual Gram matrix is non-finite";
                return false;
            }
            gram(i, j) = value;
            gram(j, i) = value;
            gram_scale = std::max(gram_scale, std::abs(value));
        }
    }
    if (!(gram_scale > 0.0) || !std::isfinite(gram_scale))
    {
        reciprocal_condition = 0.0;
        failure_reason = "Pulay residual Gram matrix has zero scale";
        return false;
    }
    ...
    for (int i = 0; i < history_size; ++i)
    {
        for (int j = i; j < history_size; ++j)
        {
            const double value = gram(i, j) / gram_scale;
            system(i, j) = value;
            system(j, i) = value;
        }
    ...
    const std::optional<matrix>& band_output)
{
    ...
    std::optional<matrix> band_residual;
    if (has_band)
    {
        band_residual = *band_output - band_input_history_.back();
        require_finite_matrix(*band_residual, "band residual");
    }
    grid_residual_history_.push_back(grid_residual);
    if (band_residual.has_value())
    {
        band_residual_history_.push_back(*band_residual);
    }
    ...
    if (pulay_ready)
    {
        ...
        if (solved)
        {
            ...
            if (!is_finite_matrix(mixed_grid) ||
                (mixed_band.has_value() && !is_finite_matrix(*mixed_band)))
            {
                solved = false;
                failure_reason =
                    "Pulay mixed Hamiltonian contains non-finite data";
            }
            else
            {
                decision.applied_mode = MixingMode::Pulay;
                if (mixed_band.has_value())
                {
