...
void test_qsgw_formula_without_hartree()
{
...
void test_hartree_delta_is_added_once()
{
    const auto hks = one_block(hermitian(1.0, 2.0, {0.0, 0.0}));
    const auto zero = one_block(hermitian(0.0, 0.0, {0.0, 0.0}));
    const auto hartree = one_block(hermitian(0.7, -0.2, {0.25, 0.5}));

    const auto result = assemble_effective_hamiltonian(
        hks, zero, zero, zero, &hartree);
    assert_close(result.at(0).at(0)(0, 0), 1.7);
    assert_close(result.at(0).at(0)(1, 1), 1.8);
    assert_close(result.at(0).at(0)(0, 1), cplxdb(0.25, 0.5));
}
...
int main()
{
    test_qsgw_formula_without_hartree();
    test_hartree_delta_is_added_once();
