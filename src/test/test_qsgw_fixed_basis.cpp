...
void test_fixed_reference_updates_live_wfc_and_velocity_with_same_unitary()
{
    MeanField reference(1, 1, 2, 2, 1);
    initialize_identity_wfc(reference);
    reference.get_eigenvals()[0](0, 0) = -0.5;
    ...
    VelocityMatrix velocity0;
    VelocityMatrix live_velocity;
    initialize_velocity_matrix(velocity0, 1, 1, 2);
    initialize_velocity_matrix(live_velocity, 1, 1, 2);
    velocity0[0][0][0](0, 0) = 1.0;
    velocity0[0][0][0](0, 1) = cplxdb(0.2, 0.3);
    velocity0[0][0][0](1, 0) = cplxdb(0.2, -0.3);
    velocity0[0][0][0](1, 1) = -1.0;
    velocity0[0][0][1](0, 1) = cplxdb(0.0, 0.7);
    velocity0[0][0][1](1, 0) = cplxdb(0.0, -0.7);
    velocity0[0][0][2](0, 0) = 0.5;
    velocity0[0][0][2](1, 1) = 1.5;
    const VelocityMatrix velocity0_before = velocity0;

    const auto result = diagonalize_in_reference_basis(
        live, reference, hamiltonian, &velocity0, &live_velocity);
    ...
    const ComplexMatrix unitary_complex = to_complex_matrix(unitary);
    const ComplexMatrix unitary_complex_dagger = transpose(unitary_complex, true);
    for (int direction = 0; direction < 3; ++direction)
    {
        const ComplexMatrix expected_velocity =
            unitary_complex_dagger * velocity0[0][0][direction] * unitary_complex;
        for (int row = 0; row < 2; ++row)
        {
            for (int column = 0; column < 2; ++column)
            {
                assert_close(live_velocity[0][0][direction](row, column),
                             expected_velocity(row, column));
            }
        }
    }
    assert_meanfield_equal(reference, reference_before);
    ...
    // not compound the first iteration's live wavefunction or velocity.
    ...
    const ComplexMatrix second_unitary_complex =
        to_complex_matrix(second_unitary);
    const ComplexMatrix second_unitary_complex_dagger =
        transpose(second_unitary_complex, true);
    for (int direction = 0; direction < 3; ++direction)
    {
        const ComplexMatrix second_expected_velocity =
            second_unitary_complex_dagger * velocity0[0][0][direction] *
            second_unitary_complex;
        for (int row = 0; row < 2; ++row)
        {
            for (int column = 0; column < 2; ++column)
            {
                assert_close(live_velocity[0][0][direction](row, column),
                             second_expected_velocity(row, column));
            }
        }
    }
    ...
    for (int direction = 0; direction < 3; ++direction)
    {
        for (int row = 0; row < 2; ++row)
        {
            for (int column = 0; column < 2; ++column)
            {
                assert_close(velocity0[0][0][direction](row, column),
                             velocity0_before[0][0][direction](row, column));
            }
        }
    }
}
...
void test_invalid_late_kpoint_does_not_partially_update_live_state()
{
    ...
    VelocityMatrix velocity0;
    VelocityMatrix live_velocity;
    initialize_velocity_matrix(velocity0, 1, 2, 2);
    initialize_velocity_matrix(live_velocity, 1, 2, 2);
    live_velocity[0][0][0](0, 0) = 7.0;
    const VelocityMatrix live_velocity_before = live_velocity;
    ...
    assert_meanfield_equal(live, live_before);
    assert_velocity_equal(live_velocity, live_velocity_before);
    ...
    assert_throws([&] {
        (void)diagonalize_in_reference_basis(
            reference, reference, valid_hamiltonian, &velocity0,
            &live_velocity);
    });
    assert_throws([&] {
        (void)diagonalize_in_reference_basis(
            live, reference, valid_hamiltonian, &velocity0, &velocity0);
    });
    assert_throws([&] {
        (void)diagonalize_in_reference_basis(
            live, reference, valid_hamiltonian, &velocity0, nullptr);
    });
    assert_throws([&] {
        (void)diagonalize_in_reference_basis(
            live, reference, valid_hamiltonian, nullptr, &live_velocity);
    });
}
...
void test_invalid_matrix_data_does_not_mutate_live_state()
{
    ...
    initialize_velocity_matrix(velocity0, 1, 1, 2);
    initialize_velocity_matrix(live_velocity, 1, 1, 2);
    live_velocity[0][0][0](0, 0) = 4.0;

    const auto assert_rejected_without_mutation = [&](const Matz& matrix) {
        SpinKMatrixMap hamiltonian;
        hamiltonian[0][0] = matrix;
        const MeanField live_before = live;
        const VelocityMatrix velocity_before = live_velocity;
        assert_throws([&] {
            (void)diagonalize_in_reference_basis(
                live, reference, hamiltonian, &velocity0, &live_velocity);
        });
        assert_meanfield_equal(live, live_before);
        assert_velocity_equal(live_velocity, velocity_before);
    };
