...
void test_pulay_grid_coefficients_are_reused_for_band_maps()
{
    MixingOptions options;
    ...
    auto grid_target1 = deep_copy_map(grid0);
    grid_target1[0][0](0, 0) = 1.0;
    auto band_target1 = deep_copy_map(band0);
    band_target1[0][0](0, 0) = 12.0;
    assert_close(grid0.at(0).at(0)(0, 0), 0.0);
    assert_close(band0.at(0).at(0)(0, 0), 10.0);
    const auto first = mixer.mix(grid_target1, band_target1);
    assert(first.decision.applied_mode == MixingMode::Linear);

    auto grid_target2 = deep_copy_map(first.grid);
    grid_target2[0][0](1, 1) = 1.0;
    auto band_target2 = deep_copy_map(*first.band);
    band_target2[0][1](0, 0) = 24.0;
    assert_close(first.grid.at(0).at(0)(1, 1), 0.0);
    assert_close(first.band->at(0).at(1)(0, 0), 20.0);
    const auto second = mixer.mix(grid_target2, band_target2);
    ...
    assert_close(second.grid.at(0).at(0)(0, 0), 0.2);
    assert_close(second.grid.at(0).at(0)(1, 1), 0.1);
    assert_close(second.band->at(0).at(0)(0, 0), 10.4);
    assert_close(second.band->at(0).at(1)(0, 0), 20.4);
}

void test_band_channel_never_changes_grid_mixing_history_or_decision()
{
    for (const MixingMode mode : {MixingMode::Linear, MixingMode::Pulay})
    {
        MixingOptions options;
        options.mode = mode;
        options.beta = 0.2;
...
int main()
{
    ...
    test_pulay_grid_coefficients_are_reused_for_band_maps();
