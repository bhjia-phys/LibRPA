    ...
    std::map<int, std::map<libri_types<int, int>::TAC, RI::Tensor<double>>> &Cs_IJ)
{
    ...
    {
        matrix_m<complex<double>> tmp_loc(C_nao_nao.nr(), C_nao_nao.nc(), MAJOR::ROW);
#pragma omp parallel for
        for (int ilo = 0; ilo != desc_nao_nao.m_loc(); ilo++)
        {
            ...
            for (int jlo = 0; jlo != desc_nao_nao.n_loc(); jlo++)
            {
                ...
                if (Cs_IJ.count(Mu) == 0) continue;
                for (const auto &JRa_tensor : Cs_IJ.at(Mu))
                {
                    const auto &JRa = JRa_tensor.first;
                    if (JRa.first != J_loc) continue;
                    tmp_loc(ilo, jlo) += JRa_tensor.second(mu_local, i_ab, j_ab) * fourier(Mu, JRa);
                }
            }
        }
        if (C_nao_nao.is_col_major())
        {
...
#pragma omp parallel for schedule(dynamic, 4) collapse(2)
    ...
    for (int i = 0; i < n_nonsingular - 1; i++)
    {
        const int ilo_body = desc_body.indx_g2l_r(i);
        const int ilo_chi0 = desc_nabf_nabf_opt.indx_g2l_r(i + 1);
        if (ilo_body < 0 || ilo_chi0 < 0) continue;
        for (int j = 0; j < n_nonsingular - 1; j++)
        {
            const int jlo_body = desc_body.indx_g2l_c(j);
            const int jlo_chi0 = desc_nabf_nabf_opt.indx_g2l_c(j + 1);
            if (jlo_body < 0 || jlo_chi0 < 0) continue;
            chi0(ilo_chi0, jlo_chi0) += body_inv(ilo_body, jlo_body);
        }
    }
