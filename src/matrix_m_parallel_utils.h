...
#pragma omp parallel for collapse(2) schedule(dynamic)
    ...
    std::vector<std::array<int, 4>> blocks;
    for (const auto &I_entry : map_lor_I_is)
    {
        int I = I_entry.first;
        for (const auto &J_entry : map_loc_J_js)
        {
            int J = J_entry.first;
            const auto &j_indices = map_loc_J_js.at(J);
            // const auto n_j = static_cast<int>(atbasis.get_atom_nb(J));
            int n_j = j_indices.size();  // use real size
            int step = n_j / subdiv;
            int resi = n_j % subdiv;
            for (int j_subdiv = 0; j_subdiv < subdiv; j_subdiv++)
            {
                int st = j_subdiv * step + std::min(j_subdiv, resi);
                int ed = st + step + (j_subdiv < resi ? 1 : 0);
                ed = std::min(n_j, ed);  // avoid ed exceed.
                blocks.push_back({I, J, st, ed});
            }
        }
    }
...
#pragma omp parallel for schedule(dynamic)
    for (size_t i_block = 0; i_block < blocks.size(); i_block++)
    {
        ...
        for (int i_idx = 0; i_idx < i_indices.size(); i_idx++)
        {
            ...
            {
                int j = j_indices[j_idx];
                int i_loc = desc.indx_g2l_r(atbasis.get_global_index(I, i));
                int l_loc = desc.indx_g2l_c(atbasis.get_global_index(J, j));
                mat(i, j) = mat_lo(i_loc, l_loc);
            }
        }
    }
